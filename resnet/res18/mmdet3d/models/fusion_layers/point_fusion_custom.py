...
class PointFusionCustom(nn.Module):
    ...
    def __init__(self,
                 ...
                 padding_mode='zeros',
                 lateral_conv=False):
        super(PointFusionCustom, self).__init__()
    ...
    def obtain_mlvl_feats(self, img_feats, pts, img_metas):
        ...
        # img_metas 是一个列表，其长度为ｂａｔｃｈ_size
        for i in range(len(img_metas)):
            # mul_batch_img_feats list(B,[C,N])
            mul_batch_img_feats.append(
                self.sample_single(img_feats[i, :, :, :], pts[i, :, :],
                                   img_metas[i]).unsqueeze(0))
        # img_pts 是每一个ｂａｔｃｈ下的点对应的特征,每一个batch中所有帧在当前阶段采样点数量是相同的
        # 所以可以cat
        # img_pts(B,N,C)
        img_pts = torch.cat(mul_batch_img_feats, dim=0)
        return img_pts
