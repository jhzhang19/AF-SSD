...
class PointFusion(nn.Module):
    ...
    def obtain_mlvl_feats(self, img_feats, pts, img_metas):
        ...
        img_feats_per_point = []
        # print("#"*10)
        # print("img_ins", len(img_ins))
        # print("img_ins shape",img_ins[0].shape, img_ins[1].shape, img_ins[2].shape, img_ins[3].shape, img_ins[4].shape)
            ...
            for level in range(len(self.img_levels)):
                mlvl_img_feats.append(
                    self.sample_single(img_ins[level][i:i + 1], pts[i][:, :3],
                                       img_metas[i]))
            # print("mlvl_img_feats:", len(mlvl_img_feats))
            mlvl_img_feats = torch.cat(mlvl_img_feats, dim=-1)
            # print("mlvl_img_feats shape:", mlvl_img_feats.shape)
